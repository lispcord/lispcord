...
(defparameter *permission-values*
  '((:create-instant-invite . #x00000001)
...
(defun permissions-overwrite (permissions overwrites)
  (if overwrites
      (permissions-add (permissions-remove permissions (deny overwrites))
                       (allow overwrites))
      permissions))
...
(defmethod has-permission ((p permissions) key &optional channel)
  ...
  (every (lambda (k) (logtest (value p) (permission-value k)))
         key))
  ...
  (let ((guild (guild member)))
    (if (owner guild)
        ...
        (let* ((role-everyone (role-everyone guild))
          ...
          (loop :for role :across (roles member) :do
            (setf permissions (permissions-add permissions (permissions role))))
  ...
  (let ((guild (guild member)))
    (if (has-permission permissions :administrator)
        ...
        (progn
          ...
          (loop :for role :across (roles member) :do
            (permissions-overwrite permissions (overwrite channel role)))
